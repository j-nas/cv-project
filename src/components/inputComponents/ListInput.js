...
class RelevantSkillsInput extends React.Component {
  ...
  render() {
    const listItemInput = this.props.listItems.map((item) => {
      return (
        <li key={item.key}>
          <input
            type="text"
            ...
            value={item.text}
          />
          <button onClick={this.props.removeListItem} id={item.key}>
            Remove
          </button>
        </li>
      );
    ...
    const listItemDisplay = this.props.listItems.map((item) => {
